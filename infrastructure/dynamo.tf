...
resource "aws_dynamodb_table" "beeg_yoshi_epl" {
  name           = "beeg-yoshi-epl"
  hash_key       = "pk"
  range_key      = "sk"
  billing_mode   = "PROVISIONED"
...
resource "aws_iam_policy" "beeg_yoshi_epl_dynamo_read" {
  name_prefix = "beeg-yoshi-dynamo-read"
  policy = jsonencode({
    Version : "2012-10-17",
    Statement : [
      {
        ...
        Resource : [
          aws_dynamodb_table.beeg_yoshi_epl.arn,
          "${aws_dynamodb_table.beeg_yoshi_epl.arn}/index/*"
        ]
      }
    ]
  })
...
resource "aws_iam_policy" "beeg_yoshi_epl_dynamo_read_write" {
  name_prefix = "beeg-yoshi-dynamo-read"
  policy = jsonencode({
    Version : "2012-10-17",
    Statement : [
